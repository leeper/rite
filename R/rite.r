rite <- function(filename=NULL, catchOutput=FALSE, evalenv=.GlobalEnv,
	...
	menuRun <- tkmenu(menuTop, tearoff = FALSE)
		...
		if(catchOutput){
			...
			tkadd(menuRun, "separator")
			tkadd(menuRun, "command", label="Install packages", command=function()
				tkinsert(out,"end",capture.output(install.packages())))
			tkadd(menuRun, "command", label="Update packages", command=function()
				tkinsert(out,"end",capture.output(update.packages(ask='graphics',checkBuilt=TRUE))))
		} else {
			tkadd(menuRun, "command", label="List all objects", command=function()
	...
	menuHelp <- tkmenu(menuTop, tearoff = FALSE)
		...
		tkadd(menuHelp, "command", label = "R language help", underline = 0, command = utils::help.start)
		tkadd(menuHelp, "separator")
